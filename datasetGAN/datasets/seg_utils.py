...
def generate_label_argmax(inputs, imsize=None, n_label=19):
    # pred_batch = []
    # for input in inputs:
    #     try:
    #         input = input.view(1, 19, imsize, imsize)
    #     except:
    #         input = input.view(1, 34, imsize, imsize)

    #     pred = np.squeeze(input.data.max(1)[1].cpu().numpy(), axis=0)
    #     pred_batch.append(pred)

    pred_batch = np.array(inputs) # B H W?
    pred_batch = torch.from_numpy(pred_batch).to('cpu')
            
    label_batch = []
    for p in pred_batch:
        if imsize is not None:
            p = p.view(1, imsize, imsize)
        label_batch.append(tensor2label(p, n_label))
...
def generate_label(inputs, imsize):
    # input: onehot version of segmentation predictin
    pred_batch = []
    for input in inputs:
        try:
            input = input.view(1, 19, imsize, imsize)
